                ...
                <x-dropdown align="right" width="80">
                    <x-slot name="trigger">
                        <button class="relative inline-flex items-center p-2 border border-transparent text-sm leading-4 font-medium rounded-full text-gray-500 bg-white hover:text-gray-700 hover:bg-gray-50 focus:outline-none transition ease-in-out duration-150">
                            ...
                            @if(Auth::user()->unreadNotifications->count() > 0)
                                <span class="absolute top-0 right-0 inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold leading-none text-white transform translate-x-1/4 -translate-y-1/4 bg-red-600 rounded-full">{{ Auth::user()->unreadNotifications->count() }}</span>
                            @endif
                        </button>
                    </x-slot>
                    ...
                    <x-slot name="content">
                        <div class="px-4 py-3 border-b border-gray-100 bg-gray-50 rounded-t-md">
                            <p class="text-sm font-bold text-gray-700">Notificaciones Recientes</p>
                        </div>
                        <div class="max-h-60 overflow-y-auto">
                            @forelse(Auth::user()->unreadNotifications as $notification)
                                <a href="{{ route('notificaciones.leer', $notification->id) }}" class="block px-4 py-3 border-b border-gray-50 hover:bg-indigo-50 transition">
                                    <p class="text-sm font-medium text-gray-800">{{ $notification->data['mensaje'] ?? 'Nueva actividad registrada.' }}</p>
                                    <p class="text-xs text-indigo-400 mt-1 font-semibold flex items-center">
                                        <span class="material-symbols-outlined text-[12px] mr-1">schedule</span>
                                        {{ \Carbon\Carbon::parse($notification->data['fecha'])->diffForHumans() }}
                                    </p>
                                </a>
                            @empty
                                <div class="px-4 py-6 text-center flex flex-col items-center">
                                    <span class="material-symbols-outlined text-gray-300 text-3xl mb-2">notifications_paused</span>
                                    <span class="text-sm text-gray-500">No hay notificaciones nuevas.</span>
                                </div>
                            @endforelse
                        </div>
                    </x-slot>
